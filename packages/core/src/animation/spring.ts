/*!
 * This code is from https://github.com/remix-run/remix/blob/377da15e83b07adf61c83858a281f82355f92ac7/packages/component/src/lib/spring.ts
 * MIT License: https://github.com/remix-run/remix/blob/377da15e83b07adf61c83858a281f82355f92ac7/LICENSE
 */
...
export interface SpringIterator extends IterableIterator<number> {
  ...
  easing: string
  /** Position at time t (ms), normalized 0→1 (can overshoot) */
  positionAt: (t: number) => number
  /** Velocity at time t (ms) in units per second */
...
export function spring(
  ...
  let { position, velocityAt, settlingTime, easing } = computeSpring(options)
  ...
  Object.defineProperties(iter, {
    ...
    positionAt: { value: position, enumerable: false },
    velocityAt: { value: velocityAt, enumerable: false },
    toString: {
      value() {
...
function computeSpring(options: SpringOptions): ComputedSpring {
  ...
  if (zeta < 1) {
    ...
    position = (t: number) => {
      ...
      return (
        1 -
        envelope *
          (((velocityMs + zeta * omega0Ms) / omegaD) * Math.sin(omegaD * t) +
            Math.cos(omegaD * t))
      )
    }
  } else if (zeta > 1) {
    ...
    position = (t: number) =>
      1 - Math.exp(-omega0Ms * t) * (1 + (velocityMs + omega0Ms) * t)
  }
  ...
  function velocityAt(t: number): number {
    if (t < velocitySampleMs) {
      return (
        ((position(velocitySampleMs) - position(0)) / velocitySampleMs) * 1000
      )
    }
    return (
      ((position(t) - position(t - velocitySampleMs)) / velocitySampleMs) * 1000
    )
  }
  ...
  return { position, velocityAt, settlingTime, easing }
}
